...
- 智能占位符与元数据处理会更新 README 中的 `{{projectName}}`，按模板配置保留 `package.json` 中指定的字段（默认至少包含 `name`），并在项目根记录 `.yxp-template.json`（包含模板信息和初始化时间）。
- `update` 命令根据模板与项目差异生成彩色 diff（含二进制检测）、支持 `--all` 全量更新与 `--skip-all` 干跑模式，并记录更新时间。
...
- `src/commands/update.ts` 主导同步流程：读取 `.yxp-template.json`、解析忽略规则、扫描差异并支持交互式或批量更新模式。
- `src/core/update/` 提供差异引擎：`scanner.ts` 枚举文件、`differ.ts` 生成待处理项目、`handlers.ts` 判定增删改��略、`applier.ts` 写入文件并结合 `progress.ts` 展示进度。
